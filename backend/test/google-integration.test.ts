...
const dbMock = {
  get: (query: string, params: any[], callback: (err: Error | null, result?: any) => void) => {
  ...
  run: function(query: string, params: any[], callback?: (this: any, err: Error | null) => void) {
    console.log('DB Mock RUN:', query, params);
    if (typeof callback === 'function') {
      callback.call({ lastID: 999, changes: 1 }, null);
    }
  }
};

// Simple mock for the Google OAuth handler
const googleOAuthHandlerMock = async (request: FastifyRequest, reply: FastifyReply) => {
  console.log('Mock Google handler called with:', request.url);

  // Determine if we're testing existing or new user
  const isExistingUser = (request.query as any).case === 'existing';
